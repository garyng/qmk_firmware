...
void HidGenericReportParser::Parse(uint16_t vid, uint16_t pid, uint8_t iface, uint8_t endpoint, uint8_t len, uint8_t *buff) { printf("vid: %X pid: %x, iface: %x, endpoint: %x", vid, pid, iface, endpoint); }
